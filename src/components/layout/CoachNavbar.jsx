...
const links = [
  ...
  { to: '/coach/find-athletes', label: 'Buscar Atletas' },
  { to: '/coach/saved-athletes', label: 'Mis Prospectos' },
  { to: '/coach/messages', label: 'Mensajes' },
]
...
export default function CoachNavbar() {
  ...
  const initials = (user?.name || 'Coach').split(' ').map((n) => n[0]).slice(0, 2).join('')
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden lg:flex items-center gap-2 relative" ref={menuRef}>
            ...
            <Link to="/coach/messages" className="p-2 rounded-full text-white/80 hover:text-white hover:bg-white/5">
            ...
            >
              <span className="w-8 h-8 rounded-full bg-electric flex items-center justify-center text-white font-semibold text-xs border-2 border-white/20">
                {initials}
              </span>
            ...
            {menuOpen && (
              <div className="absolute right-0 top-14 w-48 glass rounded-2xl shadow-glass-lg p-2 animate-scale-in">
                <Link to="/coach/dashboard" onClick={() => setMenuOpen(false)} className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm text-ink hover:bg-ink/5">
