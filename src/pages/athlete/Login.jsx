...
import { Mail, Lock, Volleyball, Info } from 'lucide-react'
...
export default function AthleteLogin() {
  ...
  const handleSubmit = (e) => {
    ...
    const result = login('athlete', { email, password })
    if (!result.success) {
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <GlassCard className="w-full max-w-md p-8 animate-scale-in">
        <div className="flex flex-col items-center mb-6">
          ...
          <h1 className="font-heading font-extrabold text-2xl text-ink">Bienvenido, Atleta</h1>
          <p className="text-sm text-ink-soft mt-1">Inicia sesión para continuar tu reclutamiento.</p>
        </div>
        ...
        <p className="text-xs text-ink-soft/70 text-center mt-5 flex items-center justify-center gap-1.5">
          <Info className="w-3.5 h-3.5" /> Demo: tomas@volleyrecruitpr.com / demo123
        </p>

        <p className="text-sm text-ink-soft text-center mt-4">
          ¿No tienes cuenta? <Link to="/athlete/register" className="text-electric font-semibold hover:underline">Crear cuenta</Link>
        </p>
        <p className="text-sm text-ink-soft text-center mt-2">
          ¿Eres entrenador? <Link to="/coach/login" className="text-electric font-semibold hover:underline">Inicia sesión aquí</Link>
        </p>
      </GlassCard>
    </div>
  )
