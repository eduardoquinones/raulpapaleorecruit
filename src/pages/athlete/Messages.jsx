...
import { athleteThreads as threads } from '../../data/mockMessages'

export default function AthleteMessages() {
  const [active, setActive] = useState(threads[0])
  const [draft, setDraft] = useState('')
  const [query, setQuery] = useState('')
  const [conversations, setConversations] = useState(threads)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6">
        ...
        <GlassCard className="p-6 flex flex-col h-[520px]">
          <div className="border-b border-ink/10 pb-3 mb-4 flex items-center gap-3">
            <span className="relative shrink-0">
              <span className="w-10 h-10 rounded-full bg-electric flex items-center justify-center text-white font-semibold text-xs">
                {activeThread.from.replace('Coach ', '').split(' ').map((n) => n[0]).slice(0, 2).join('')}
              </span>
