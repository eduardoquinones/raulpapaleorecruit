...
import { Mail, Lock, User, Volleyball } from 'lucide-react'
...
export default function AthleteRegister() {
  ...
  const handleSubmit = (e) => {
    ...
    const result = register('athlete', { name: form.name, email: form.email, password: form.password })
    if (!result.success) {
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <GlassCard className="w-full max-w-md p-8 animate-scale-in">
        <div className="flex flex-col items-center mb-6">
          <span className="w-12 h-12 rounded-full bg-deep text-gold flex items-center justify-center mb-3">
            <Volleyball className="w-6 h-6" />
          </span>
          <h1 className="font-heading font-extrabold text-2xl text-ink">Crea tu Perfil de Atleta</h1>
          <p className="text-sm text-ink-soft mt-1">Empieza tu camino al voleibol universitario.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <Button type="submit" variant="gold" className="w-full mt-2">Crear Cuenta</Button>
        </form>

        <p className="text-sm text-ink-soft text-center mt-6">
          ¿Ya tienes cuenta? <Link to="/athlete/login" className="text-electric font-semibold hover:underline">Iniciar sesión</Link>
        </p>
        <p className="text-sm text-ink-soft text-center mt-2">
          ¿Eres entrenador? <Link to="/coach/register" className="text-electric font-semibold hover:underline">Crea tu cuenta aquí</Link>
        </p>
      </GlassCard>
    </div>
  )
